...
2. If available, use the `listComments` tool to list all available comments
3. Understand the intent behind each piece of feedback
4. Make the requested changes to address the feedback
5. When feedback has been tackled, use the `resolveComments` tool if available to mark those comments as resolved, or the `deleteComments` tool to delete them
